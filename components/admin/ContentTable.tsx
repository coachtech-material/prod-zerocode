  ...
  return (
    <table className="w-full text-sm">
      <thead className="bg-white text-slate-600">
        <tr>
          <th className="text-left px-3 py-2 w-28">キー</th>
          ...
          <th className="text-left px-3 py-2 w-32">所要時間</th>
        </tr>
      </thead>
      <tbody>
        {chapters.map((ch) => (
          <>
            <tr key={ch.id} className="border-t border-brand-sky/20 bg-white">
              ...
              <td className="px-3 py-3 align-middle">
                <Link href={`/admin/courses/${courseId}/chapters/${ch.id}`} className="underline decoration-white/20 hover:decoration-white">
              ...
              <td className="px-3 py-3 align-middle">
                <span className="rounded-full bg-brand-sky/10 px-2 py-0.5 text-xs">{ch.status === 'published' ? '公開' : '非公開'}</span>
              </td>
              <td className="px-3 py-3 align-middle">{chapterDuration(ch.id)} 分</td>
            </tr>
            {open[ch.id] && (grouped.get(ch.id) || []).map((s) => (
              <tr key={s.id} className="border-t border-brand-sky/20">
                <td className="px-3 py-3 pl-9 align-middle">{s.section_sort_key}</td>
                <td className="px-3 py-3 align-middle">
                  <Link href={`/admin/courses/${courseId}/sections/${s.id}`} className="underline decoration-white/20 hover:decoration-white">
                ...
                <td className="px-3 py-3 align-middle">
                  <span className="rounded-full bg-brand-sky/10 px-2 py-0.5 text-xs">{s.status === 'published' ? '公開' : '非公開'}</span>
                </td>
        ...
        {!chapters.length && (
          <tr>
            <td className="px-3 py-6 text-slate-500" colSpan={4}>このコースにコンテンツはありません。</td>
          </tr>
        )}
      </tbody>
    </table>
